...
### Svelte 5 Reactivity & Conditional Blocks

This section documents critical implementation details for the IR tree's reactivity system in Svelte 5.

#### The Problem

When components are inside `{#if}` blocks, toggling from `false` → `true` would not re-render the content:

```svelte
{#if showText}
  <Text content="Hello" />  <!-- Would not appear when showText becomes true -->
{/if}
```

**Root cause:** Svelte 5's `$state` proxies don't trigger reactivity when mutating arrays via `push()`. The `$state.snapshot()` used in `Render.svelte` doesn't track internal array mutations—only reassignment of the root state variable triggers updates.

#### The Solution

A three-part solution ensures both SSR compatibility and client-side conditional reactivity:

**1. Tree Version Counter (`Render.svelte`)**
...
const collector: Collector = {
  ...
  markDirty() { treeVersion++ }  // Increment to signal tree change
}

$effect(() => {
  treeVersion  // Track the version
  // Re-render when version changes
  result = renderTree(root, ...)
})
```

**2. Hybrid Registration Pattern (all element components)**
...
**Why both sync AND effect?**
- **Sync `addChild`**: Required for `render()` SSR—effects don't execute during server-side rendering
- **Effect body `addChild`**: Required for correct client-side ordering when conditionals toggle
- **Effect cleanup `removeChild`**: Removes node when component unmounts (e.g., `{#if}` becomes false)
- **`untrack()`**: Prevents infinite loops—without it, `markDirty()` → `treeVersion++` → effect re-runs → `addChild` → `markDirty()` → ∞

**3. Idempotent `addChild` with Marker-Based Identity (`context.ts`)**

```ts
export function addChild(parent, node, markerId, collector) {
  const children = parent?.children
  if (!children) return
  
  // Skip if already registered (idempotent for hybrid pattern)
  if (markerId && children.some((c) => c._markerId === markerId)) return
  
  node._markerId = markerId
  children.push(node)
  collector?.markDirty()
}

export function removeChild(parent, markerId, collector) {
  const children = parent?.children
  if (!children || !markerId) return
  
  // Use markerId comparison, NOT proxy reference (proxies create new references)
  const index = children.findIndex((c) => c._markerId === markerId)
  if (index !== -1) {
...
- `$state` objects are proxies—each access can return a different reference
- Direct reference comparison (`children.indexOf(node)`) fails with proxies
- String-based `markerId` provides stable identity across proxy accesses

#### Why Effect Body Must Call `addChild`

During Svelte's effect flush cycle:
1. Effect cleanups run FIRST (for all effects being cleaned up)
2. Effect bodies run AFTER (for all effects being created/re-run)

If `addChild` is only in the sync code (outside effect):
- Component A unmounts → cleanup runs `removeChild(A)`
- Component B mounts → sync `addChild(B)` already ran during init
- Result: B was added before A was removed, tree order is wrong

If `addChild` is also in effect body:
- Component A unmounts → cleanup runs `removeChild(A)` 
- Component B mounts → effect body runs `addChild(B)` (idempotent, re-adds if needed)
- Result: Correct ordering because effect body runs after cleanups

#### Development Guidelines

**When creating new element components:**

1. Always use the hybrid registration pattern (copy from `Text.svelte`)
2. Generate a unique `markerId` via `generateMarkerId()`
3. Wrap effect body and cleanup in `untrack()` to prevent infinite loops
4. Never compare `$state` proxies by reference—use `markerId`

**Common mistakes to avoid:**

| ❌ Don't | ✅ Do |
|----------|-------|
| `children.indexOf(node)` | `children.findIndex(c => c._markerId === markerId)` |
| `addChild` only in sync code | Sync `addChild` + effect body `addChild` |
| `$effect(() => { addChild(...) })` without untrack | `$effect(() => { untrack(() => addChild(...)) })` |
| `onDestroy(() => removeChild(...))` | Effect cleanup (captures correct markerId) |

**Testing conditional reactivity:**
...
<Email>
  {#if show}
    <Text content="This should appear/disappear correctly" />
  {/if}
</Email>
```

Both buttons should work—content appears on Show, disappears on Hide, and reappears on Show again.
